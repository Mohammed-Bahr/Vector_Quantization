...
#         self.Centroid = self.compute_average_block()

     
#     def image_to_blocks(self):
#         """
#         تقسم الصورة إلى بلوكات (vectors) بحجم block_height × block_width
#         وترجعها في شكل 2D list.

#         image_path: مسار الصورة
#         block_width: عرض كل بلوك
#         block_height: ارتفاع كل بلوك
#         """
#         image_path = self.path
#         block_width = self.block_w
#         block_height = self.block_h
#         # فتح الصورة وتحويلها لتدرج رمادي (اختياري)
...
#                 # لو آخر بلوك أصغر، نكمّله بالصفار
...
#     def compute_average_block():
#         """
#         blocks: list of blocks (each block is numpy array with same shape)
        
#         ترجع بلوك متوسط ناتج عن جمع كل القيم في نفس المكان
#         ثم قسمتها على عدد البلوكات.
#         """

#         # تحويل blocks إلى array ثلاثي الأبعاد
#         blocks_array = np.array(self.Blocks)

#         # حساب المتوسط على محور البلوكات
#         avg_block = np.mean(blocks_array, axis=0)

#         # تقريب القيم لأقرب عدد صحيح (اختياري)
#         # avg_block = np.round(avg_block).astype(int)

#         return avg_block


#     def block_difference(block1, block2):
#         """
#         تحسب الفرق بين بلوكين نفس الحجم عن طريق طرح كل عنصر من الثاني
#         وأخذ القيمة المطلقة، ثم جمع كل الفروق.
#         """
#         total_diff = 0
...
#         for i in range(rows):
#             for j in range(cols):
#                 diff = abs(block1[i][j] - block2[i][j])
#                 total_diff += diff

#         return total_diff


#     def generate_codebook(self, k):
#         """
#         k: عدد البلوكات في الكودبووك
        
#         تنشئ كودبووك من خلال اختيار k بلوكات عشوائية من البلوكات الموجودة.
#         """
